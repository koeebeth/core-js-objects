...
function mergeObjects(objects) {
  const merged = {};
  const entries = [];
  objects.forEach((obj) => entries.push(...Object.entries(obj)));
  entries.forEach((entry) => {
    if (merged[entry[0]] !== undefined) {
...
function removeProperties(obj, keys) {
  const modified = obj;
  keys.forEach((key) => delete modified[key]);
  return modified;
}
...
function makeWord(lettersObject) {
  let max = 0;
  Object.values(lettersObject).forEach((i) => {
    i.forEach((idx) => {
      if (idx > max) max = idx;
    });
  });
  const word = new Array(max);
